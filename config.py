...
class Config:
    ...
    def __init__(self):
        self.themes = []
        self._add_themes()
        self.idx = 0                        # idx doit être défini AVANT d'accéder à self.themes[self.idx]
        self.theme = self.themes[self.idx]
        self.font = pygame.font.SysFont('monospace', 18, bold=True)

    def change_theme(self):
        self.idx += 1
        self.idx %= len(self.themes)  # permet plusieurs choses importantes : avoir le nb de theme qu on veut et de plus grace au % quand on arrive a la fin on revient au debut
        self.theme = self.themes[self.idx]
